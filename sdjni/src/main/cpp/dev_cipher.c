...
static int cipher_one_test(sdkey_data_t *data, const char *name, ULONG hwId)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_Decrypt ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    rv = SKF_CloseHandle(hKey);
    if (rv != SAR_OK) {
        LOGI("SKF_CloseHandle ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    hKey = NULL;

    if (inLen != decLen || memcmp(in, dec, inLen) != 0) {
        LOGE("cipher[%s] one time ERROR\n", name);
        LOGI("plain data is:\n");
        ShwHexBuf(in, inLen);
        LOGI("decrypt data is:\n");
        ShwHexBuf(dec, decLen);
        goto error;
    }

    LOGI("cipher[%s] one time ok\n", name);
...
static int cipher_three_test(sdkey_data_t *data, const char *name, ULONG hwId)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        if (rv != SAR_OK) {
            LOGE("SKF_EncryptUpdate ERROR, errno[0x%08x]\n", rv);
            goto error;
        }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_EncryptFinal ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    penc += left;
    encLen = penc - enc;
    ...
    rv = SKF_DecryptInit(hKey, param);
    if (rv != SAR_OK) {
        LOGE("SKF_DecryptInit ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if ((inLen * 3) != decLen || memcmp(in, dec, inLen) != 0) {
        LOGE("cipher[%s] three time enc/dec ERROR\n", name);
        LOGI("plain data len: %ld\n", decLen);
        ShwHexBuf(in, inLen);
        LOGI("decrypt data len: %ld\n", decLen);
        ShwHexBuf(dec, decLen);
        goto error;
    }
    LOGI("cipher[%s] three time enc/dec ok\n", name);
...
static void cipher_test(sdkey_data_t *data, const char *name, ULONG hwId)
{
    if (!algIsSupported(hwId)) {
        LOGI("cipher algorithm[%s] is UNSUPPORTED\n", name);
        return;
    }
