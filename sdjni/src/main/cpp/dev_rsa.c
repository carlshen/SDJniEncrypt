...
static int generateSignKeyPair(HCONTAINER hCon, ULONG BitsLen)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_GenRSAKeyPair ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
static int importEncKeyPair(DEVHANDLE hDev, HCONTAINER hCon)
{
    ...
    if (!pad) {
        LOGE("malloc keyPad ERROR\n");
        goto error;
    }
    if (padding_PKCS1_type2(pad, padLen, key, keyLen) <= 0) {
        LOGE("padding_PKCS1_type2 ERROR\n");
        goto error;
    }
    ...
    rv = SKF_ExtRSAPubKeyOperation(hDev, &sign_pub, pad, padLen,
            enc, &encLen);
    if (rv != SAR_OK) {
        LOGE("SKF_ExtRSAPubKeyOperation ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (!RSA_generate_key_ex(rsa, sign_pub.BitLen, bn, NULL)) {
        LOGE("RSA_generate_key_ex ERROR\n");
        goto error;
    }
    dlen = i2d_RSAPrivateKey(rsa, &der);
    if (dlen <= 0) {
        LOGE("i2d_RSAPrivateKey ERROR\n");
        goto error;
    }
#else
    rv = SKF_GenExtRSAKey(hDev, sign_pub.BitLen, &enc_pri);
    if (rv != SAR_OK) {
        LOGE("SKF_GenExtRSAKey ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
//#error "You need to der encode the enc_pri"
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ImportRSAKeyPair ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
static int rsaEncDec(DEVHANDLE hDev, ULONG bitsLen)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ExtRSAPriKeyOperation ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    if (decLen != padLen || memcmp(dec, pad, decLen)) {
        LOGE("rsa enc/dec ERROR\n");
        LOGI("plain_text:\n");
...
static int rsaSessionCipher(DEVHANDLE hDev, HCONTAINER hCon1, HCONTAINER hCon2)
{
    ...
    if ((dec_len != plain_len) || memcmp(dec_txt, plain_txt, dec_len)) {
        LOGE("rsa plain text and dec text is different\n");
        LOGI("plain_txt:\n");
...
static int rsa_test(sdkey_data_t *data, ULONG bitsLen)
{
    ...
    LOGI("rsa[%ld] generate rsa sign key pair ok\n", bitsLen);
    ...
    LOGI("rsa[%ld] import encrypt key pair ok\n", bitsLen);
    ...
    LOGI("rsa[%ld] get container typte ok\n", bitsLen);

    /* test sign/verify */
    if (!rsaSignVerify(data->hDev, data->hCon1)) {
        LOGE("rsa[%ld] sign/verify ERROR\n", bitsLen);
    } else {
        LOGI("rsa[%ld] sign/verify ok\n", bitsLen);
    }

    /* test enc/dec */
    if (!rsaEncDec(data->hDev, bitsLen)) {
        LOGE("rsa[%ld] encrypt/decrypt ERROR\n", bitsLen);
    } else {
        LOGI("rsa[%ld] encrypt/decrypt ok\n", bitsLen);
    }

    /* test export/import session key and cipher enc/dec */
    if (!rsaSessionCipher(data->hDev, data->hCon1, data->hCon2)) {
        LOGE("rsa[%ld] export/import session ERROR\n", bitsLen);
        goto error;
    } else {
        LOGI("rsa[%ld] export/import session ok\n", bitsLen);
    }
...
int dev_rsa_test(sdkey_data_t *data)
{
    if (!algIsSupported(SGD_RSA)) {
        LOGI("rsa algorithm is UNSUPPORTED\n");
        return 0;
    }
