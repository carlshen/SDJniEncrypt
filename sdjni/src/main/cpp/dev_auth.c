...
int dev_auth_test(sdkey_data_t *data)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_SetSymmKey ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_CloseHandle ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_DevAuth ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ChangeDevAuthKey ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_CreateApplication ERROR, errno[0x%08x]\n", rv);
        data->hApp = NULL;
        goto error;
    }
    LOGI("create application[%s] ok\n", APP_NAME);
    ...
    if (rv != SAR_OK) {
        LOGE("verify pin code ERROR\n");
        goto error;
    }
    LOGI("verify application[%s] pin ok\n", APP_NAME);

    rv = SKF_ClearSecureState(data->hApp);
    if (rv != SAR_OK) {
        LOGE("SKF_ClearSecureState ERROR, errno[0x%08x]\n", rv);
        data->hApp = NULL;
        goto error;
    }
    LOGI("clear application[%s] secure state ok\n", APP_NAME);
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_CloseApplication ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    data->hApp = NULL;
    LOGI("close application[%s] ok\n", APP_NAME);

    listLen = sizeof (appList) / sizeof (CHAR);
    rv = SKF_EnumApplication(data->hDev, appList, &listLen);
    if (rv != SAR_OK) {
        LOGE("SKF_EnumApplication ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    if (!NameListFind(appList, listLen, APP_NAME)) {
        LOGE("Find app name[%s] ERROR\n", APP_NAME);
        goto error;
    }

    rv = SKF_OpenApplication(data->hDev, APP_NAME, &data->hApp);
    if (rv != SAR_OK) {
        LOGE("SKF_OpenApplication ERROR, errno[0x%08x]\n", rv);
        data->hApp = NULL;
        goto error;
    }
    LOGI("open application[%s] ok\n", APP_NAME);
