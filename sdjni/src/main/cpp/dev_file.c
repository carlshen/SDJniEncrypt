...
int dev_file_test(sdkey_data_t *data)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_CreateFile ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    LOGI("create file[%s] ok\n", FILE_NAME);

    listLen = sizeof (fileList) / sizeof (CHAR);
    rv = SKF_EnumFiles(data->hApp, fileList, &listLen);
    if (rv != SAR_OK) {
        LOGE("SKF_EnumFiles ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    if (!NameListFind(fileList, listLen, FILE_NAME)) {
        LOGE("Find file name[%s] ERROR\n", FILE_NAME);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_GetFileInfo ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_WriteFile ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ReadFile ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    if (wLen != rLen) {
        LOGE("read len(%ld) != write len(%ld) ERROR\n", rLen, wLen);
        goto error;
    }

    if (memcmp(wdata, rdata, wLen) != 0) {
        LOGE("read/write data compare ERROR\n");
        LOGI("Write data is:\n");
...
error:
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_DeleteFile ERROR, errno[0x%08x]\n", rv);
    }
    LOGI("delete file[%s] ok\n", FILE_NAME);
