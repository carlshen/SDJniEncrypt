...
static int generateSignKeyPair(HCONTAINER hCon)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_GenECCKeyPair ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
static int importEncKeyPair(DEVHANDLE hDev, HCONTAINER hCon)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_GenRandom ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ExtECCEncrypt ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_EncryptInit ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    rv = SKF_Encrypt(hKey, enc_pri.PrivateKey, sizeof (enc_pri.PrivateKey),
            cip, &cipLen);
    if (rv != SAR_OK) {
        LOGE("SKF_Encrypt ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ImportECCKeyPair ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
static int eccSignVerify(DEVHANDLE hDev, HCONTAINER hCon)
{
    ...
    if (!algIsSupported(SGD_SM2_1)) {
        LOGI("ecc ecdsa algorithm is UNSUPPORTED\n");
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_RSASignData ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    rv = SKF_ECCVerify(hDev, &sign_pub, in, inLen, &sig);
    if (rv != SAR_OK) {
        LOGE("SKF_RSAVerify ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
static int eccEncDec(DEVHANDLE hDev)
{
    ...
    if (!algIsSupported(SGD_SM2_3)) {
        LOGI("ecc ecies algorithm is UNSUPPORTED\n");
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ExtECCDecrypt ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    if (decLen != inLen || memcmp(in, dec, inLen)) {
        LOGE("ecc plain text && dec text compare ERROR\n");
        LOGI("plain text:\n");
        ShwHexBuf(in, inLen);
        LOGI("dec text:\n");
...
static int eccSessionCipher(DEVHANDLE hDev, HCONTAINER hCon1, HCONTAINER hCon2)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_ExportPublicKey ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_RSAExportSessionKey ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    rv = SKF_ImportSessionKey(hCon2, algId, 
            ...
            &hKey2);
    if (rv != SAR_OK) {
        LOGE("SKF_ImportSessionKey ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("hKey1 SKF_Encrypt ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("hKey2 SKF_Decrypt ERROR, errno[0x%08x]\n", rv);
        goto error;
    }

    if ((dec_len != plain_len) || memcmp(dec_txt, plain_txt, dec_len)) {
        LOGE("plain text and dec text is different\n");
        LOGI("plain_txt:\n");
...
static int eccAgreement(DEVHANDLE hDev, HCONTAINER hCon1, HCONTAINER hCon2)
{
    ...
    if (!algIsSupported(SGD_SM2_2)) {
        LOGI("ecc ecdh algorithm is UNSUPPORTED\n");
        goto error;
    }
    ...
    rv = SKF_GenerateAgreementDataWithECC(hCon1, algId,
            ...
            &hAgr);
    if (rv != SAR_OK) {
        LOGE("SKF_GenerateAgreementDataWithECC ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    rv = SKF_GenerateAgreementDataAndKeyWithECC(hCon2, algId,
            ...
            &hKey2);
    if (rv != SAR_OK) {
        LOGE("SKF_GenerateAgreementDataAndKeyWithECC ERROR, errno[0x%08x]\n", 
                rv);
        goto error;
    }
    ...
    rv = SKF_GenerateKeyWithECC(hAgr,
            ...
            &hKey1);
    if (rv != SAR_OK) {
        LOGE("SKF_GenerateKeyWithECC ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
...
int dev_ecc_test(sdkey_data_t *data)
{
    ...
    if (!generateSignKeyPair(data->hCon1)) {
        LOGE("generateSignKeyPair ERROR\n");
        goto error;
    }
    ...
    LOGI("ecc import ecies key pair ok\n");

    rv = SKF_GetContainerType(data->hCon1, &conType);
    if (rv != SAR_OK || conType != 2) {
        LOGE("SKF_GetContainerType ERROR, errno[0x%08x], type[0x%08x]\n", 
                rv, conType);
        goto error;
    }
    LOGI("ecc get container type ok\n");

    /* test sign/verify */
    if (!eccSignVerify(data->hDev, data->hCon1)) {
        LOGE("ecc sign/verify ERROR\n");
    } else {
        LOGI("ecc sign/verify ok\n");
    }

    /* test encrypt/decrypt */
    if (!eccEncDec(data->hDev)) {
        LOGE("ecc encrypt/decrypt ERROR\n");
    } else {
        LOGI("ecc encrypt/decrypt ok\n");
    }

    /* test export/import session */
    if (!eccSessionCipher(data->hDev, data->hCon1, data->hCon2)) {
        LOGE("ecc export/import session ERROR\n");
    } else {
        LOGI("ecc export/import session ok\n");
    }

    if (!eccAgreement(data->hDev, data->hCon1, data->hCon2)) {
        LOGE("ecc agrement ERROR\n");
        goto error;
    } else {
        LOGI("ecc agrement ok\n");
    }
