        ...
        unsigned char *out, size_t *outlen)
{
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_DigestInit ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    ...
    if (rv != SAR_OK) {
        LOGE("SKF_Digest ERROR, errno[0x%08x]\n", rv);
        goto error;
    }
    *outlen = len;
...
#endif
{
    ...
    if (devLen != softLen || memcmp(dev_out, soft_out, devLen) != 0) {
        LOGE("hash[%s] one time ERROR\n", name);
        LOGI("hardware hash[%s]:\n", name);
        ShwHexBuf(dev_out, devLen);
        LOGI("software hash[%s]:\n", name);
        ShwHexBuf(soft_out, softLen);
    }
#endif

    LOGI("hash[%s] one time ok\n", name);
...
#endif
{
    if (!algIsSupported(hwId)) {
        LOGI("hash algorithm[%s] is UNSUPPORTED\n", name);
        return;
    }
